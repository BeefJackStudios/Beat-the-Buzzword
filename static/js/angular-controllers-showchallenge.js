...
function ShowChallengesController($scope, $timeout, $location, sharedData, serverLayer, sharedUtilities) {
	...
	$scope.doLoad = function(category) {
		...
		if(sharedData.currentChallengeUserId) {
			...
			if (sharedData.hiscorePlayerId){
				var url = BTBW.CONST.BASE_URL+"/php/functions.php?mode=getUserScores&nocache="+nocache+"&playerId="+sharedData.hiscorePlayerId+"&category="+category;
			} else {
				var url = BTBW.CONST.BASE_URL+"/php/functions.php?mode=getScores&nocache="+nocache+"&playerId="+sharedData.hiscorePlayerId+"&category="+category;
			}
				...
				for (var i in spl){
					...
					if (spl2[0]){ //  && spl2[1] && spl2[2]
						...
						if (connection){
							...
							//var name = spl2[1]; // $scope.getConnectionById(spl2[0]).name;
							//var picture = spl2[2]; // $scope.getConnectionById(spl2[0]).picture;
							
							var name = $scope.getConnectionById(spl2[0]).name;
							var picture = $scope.getConnectionById(spl2[0]).picture;
							
							var chall = {playerId:spl2[0], picture:picture, name:name, category:spl2[3], score:spl2[4], rank:"Intern"};
							temp.push(chall);
